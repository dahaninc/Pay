...
| **Resend** | Create API key → `RESEND_API_KEY`. Verify the `paypigeon.io` sending domain and set `PAYPIGEON_FROM_EMAIL=PayPigeon <info@paypigeon.io>`. Add webhook → `https://<domain>/api/webhooks/resend` (delivered/opened/clicked/bounced). |
